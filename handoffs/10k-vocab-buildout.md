...
## 對話 4 紀錄(2026-05-13 接續)

**起點**:5,080 字 / 50.8%
**終點**:5,763 字 / 57.6%
**新增**:+683 字(其中 -1 sight-words 推到 499 是修正計算後)

### 批次明細

| 批次 | 主題 | 字數 | 累積 |
|------|------|------|------|
| D2 | sight-words 推到 499 + literature 新主題 | +133(+50 sw / +83 lit) | 5,213 |
| D3 | AWL Sublist 1+2 (academic-words 新主題) | +81 | 5,294 |
| D4 | AWL Sublist 3+4 | +82 | 5,376 |
| D5 | AWL Sublist 5+6 部分 | +71 | 5,447 |
| D6 | AWL Sublist 6 餘+7 | +47 | 5,494 |
| D7 | AWL Sublist 8+9+10 完成 | +103 | 5,597 |
| D8 | phrasal-verbs 從 75 推到 177 | +102 | 5,699 |
| D9 | idioms 新主題(64 條常用慣用語) | +64 | 5,763 |

### 主題狀態

新增主題(已寫入 themes.json):
- **academic-words**(AWL 學術詞)— 384 字,AWL Sublist 1-10 主要詞完成
- **literature**(文學)— 83 字,常見文學/修辭/語言學詞彙
- **idioms**(慣用語)— 64 條,常用英文諺語/俗語

擴充主題:
- **sight-words** 449 → 499(目標 500 達成,-1 因為計數細節)
- **phrasal-verbs** 75 → 177(常用片語動詞補完)

### 還剩什麼

到 10,000 字還缺 ~4,237 字。建議方向(可依需求調整):

1. **既有主題擴充**(高 ROI):
   - countries 15 → 60-80(常見國家+國籍)
   - tools 9 → 40-50
   - materials 5 → 30-40
   - idioms 64 → 150-200
   - phrasal-verbs 177 → 250-300
   - 約 +400-500 字
...
3. **真正硬功夫**:
   - 在常用詞典中找出剩餘的高頻字(2000-3000 frequency rank)
   - 補完不常見但讀書必遇:科學進階、商業進階、醫學進階
   - 約 +2,500-3,000 字

### 接手對話的下一步(D10 起)

選一條路:
- **快路徑**:擴充既有主題到目標尺寸,加上抽象詞、變化形,1-2 個對話可推到 8,000-8,500
- **慢路徑**:逐項補完冷僻詞,2-3 個對話才到 10,000(但覆蓋更廣)

學習者偏好 input(看影片讀文章),高頻優先永遠對。建議走快路徑為主,真到 10,000 補漏即可。
